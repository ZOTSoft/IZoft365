function show_warehouse_tree( table ){
    ...
    $.ajax({ 
      ...
      dataType: 'json'
        ...
        gridUrl = 'warehouse/warehouse.php?do=get&table=' + dataz.name;
...
function getMultip( elem ){
    ...
    if ( mId != '' && iId != '' ){
        $.ajax({
            type: "POST",
            url: "warehouse/warehouse.php?do=getMultip",
            data: {
                itemid: iId,
                measureid: mId
            }
...
function viewA( table, iddoc ){
    ...
    if ( id != null || $( '#table-' + table ).myTreeView( 'getSelected' ) != null){
        ...
        $.ajax({
            url: 'warehouse/warehouse.php?do=view_el&table=' + table + '&id=' + id
...
function createA( table, iddoc ){
    ...
    $.ajax({
        url: 'warehouse/warehouse.php?do=create_el&table=' + table + '&parentid=0' + iddocstr  
    }).success( function ( form ){
        $( "body" ).append( form );   
...
//                if ( r ){  
        ...
        if ( editable == 1 ){
            ...
            $.ajax({
                type: 'POST',
                url: 'warehouse/warehouse.php?do=edit_el&table=' + table + '&id=' + id,
...
function createBasedA( baseDoc, id, newDoc ){    
    ...
    $.ajax({
        url: 'warehouse/warehouse.php?do=create_based_doc&basedoc=' + baseDoc + '&id=' + id + '&newdoc=' + newDoc 
    }).success( function ( form ){
        $( "body" ).append( form );   
...
function ZConductInventoy( baseDoc, id ){    
    $.ajax({
        url: 'warehouse/warehouse.php?do=ZConductInventoy&basedoc=' + baseDoc + '&id=' + id 
...
function deleteA( table, mode ){    
    if ( $( '#table-' + table ).myTreeView( 'getSelected' ) != null ){
        ...
        if ( op != 1 ){
            $.post( 'warehouse/warehouse.php?do=getcounts&table=' + table, { id: id } ).success( function ( dataz ) {
                if ( dataz == 0 ){
                    var name = '';
                    ...
                    bootbox.confirm( "Вы действительно хотите удалить " + name + "?", function ( r ){
                        if ( r ){
                            if ( table.substring( 0, 2 ) == 't_' ){
                                ...
                                $.post( 'warehouse/warehouse.php?do=delete', { id: id, table: table } ).success( function ( dataz ) {
...
function groupConduct(){
    $.ajax({ 
        type: "POST",
        url: "warehouse/warehouse.php?do=conductOrders"
    }).success( function ( data ){
        data = $.parseJSON( data );
...
function showBarcodeSeeker( elem ){
    ...
    $.ajax({ 
        type: "POST",
        url: 'warehouse/warehouse.php?do=barcodeSeeker',
        data: { sender: sender }
    }).success( function ( form ){
        $( "body" ).append( form );   
...
function getMeasure( elem ){
    ...
    if ( iId != '' && mId == '' ){
        $.ajax({
            type: "POST",
            url: "warehouse/warehouse.php?do=getMeasure",
            data: {
                itemid: iId
            }
        }).success( function ( data ){
...
function getRemains( table, mode ){
    ...
    url = "warehouse/warehouse.php?do=get&table=t_inventory&remains=" + mode + "&warehouse=" + warehouse+"&zdocid="+docid+'&dt='+dt;
...
function getReRemains(table){
    ...
    url = "warehouse/warehouse.php?do=get&table=t_inventory&idfield=documentid&documentid="+docid+'&warehouse=' + warehouse+'&nolimit=topgear&zreset=1'+'&dt='+dt;
...
function cancelConductA( btn, table, id ){
    bootbox.confirm( "Вы действительно хотите отменить проведение документа?", function ( r ){
        if ( r ){
            ...
            $.ajax({
                type: "POST",
                url: "warehouse/warehouse.php?do=cancelConduct",
                data: {
                    table: table,
                    id: id
                }
