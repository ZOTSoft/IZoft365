...
function mtv_view_el(el,id){
    ...
    if (id){
        $.ajax({
              url: 'ajax.php?do=view_el&table='+tablename+'&id='+id,
              dataType:'json'
...
function mtv_create_el(el,field,id){
    ...
    if (table=='s_note'){
       itemid='&idfield=itemid&itemid='+(table.myTreeView('getSelected').id); 
        ...
        $.ajax({ 
          url: 'ajax.php?do=zcreate&table='+tablename+parent+itemid,
          dataType:"html"
        }).success(function(dataz) {
...
function mtv_create_group(el){
        ...
        $.ajax({ 
          url: 'ajax.php?do=zgroup_create&table='+tablename+'&parentid='+parentid,
          dataType:"html"
        }).success(function(dataz) {
               bootbox.dialog({
                  message: dataz,
...
function mtv_create_elcopy(el){
    ...
    $.ajax({
        url: 'ajax.php?do=create_elcopy&table='+tablename+'&id='+id  
...
function mtv_edit(el){
    ...
    if (table.myTreeView('getSelected')!=null){
        ...
        $("body").append('<div id="dialog_edit-'+tablename+'"></div>');
         $.ajax({ 
          url: 'ajax.php?do=zedit&table='+tablename+'&id='+id,
          dataType:"html"
        }).success(function(dataz) {
                bootbox.dialog({
                  message: dataz,
...
function mtv_delete(el){
    ...
    if (table.myTreeView('getSelected')!=null){
        id=table.myTreeView('getSelected').id;
        //console.log(id);
        $.post("ajax.php?do=getcounts&table="+tablename, { id: id}).success(function(dataz) {
            if (dataz==0){
                bootbox.confirm("Вы действительно хотите удалить <b>"+table.myTreeView('getSelected').name+"</b>", function(r) {
                    if (r){ 
                        $.post("ajax.php?do=delete&table="+tablename, { id: id,table:tablename}).success(function(dataz) {
...
function deletez_ok2(){
    ...
    $.post("ajax.php?do=delete2", { id: id,menuid:menuid}).success(function(dataz) {
        //console.log(dataz);
...
function truncate_menu(table){
        ...
        bootbox.confirm('Вы действительно хотите очистить содержимое?',function(r){  
            ...
            $.ajax({
                url: 'ajax.php?do=truncate_menu&id='+id+'&menuid='+menuid,
                dataType: 'html' 
            }).success(function() {
                //console.log(data);
...
function perenoska_delai(){
    ...
    if (r){ 
         ...
         $.ajax({ 
          type: "POST",
          url: "ajax.php?do=perenoska&menuid="+id+"&id="+parentid+"&to="+to,
          dataType: 'html'
        }).success(function(dataz) {       
        //console.log(dataz);
...
$('#loading').show();
    ...
    switch(otchet){
        ...
        case 'gethtml_anal_sale': title='Отчет по движению документов'; break;
        ...
        case 'gethtml_anal_sale2': title='Анализ продаж'; break;
    }
...
function dez_menu_print(el){
    menuid=$('#selc').val();
    window.open('ajax.php?do=print_dez_menu&type=html&menu_id='+menuid,'','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no');
}
...
/*        if ($('#table-'+table).myTreeView('getSelected')!=null){
        ...
        */
        ...
        $.ajax({ 
          url: 'ajax.php?do=zgroup_create&table='+table+'&parentid='+parentid,
          dataType:"html"
        }).success(function(dataz) {
...
function approve_mail(){
    $.ajax({ url: "ajax.php?do=approve_mail",dataType:"html"}).success(function(dataz) {
        alert(dataz);
    });
}
