...
switch ($connection){
	...
	case 'PanelZ' :
			$answer = array();
			if(countCompliteZ(definitionSESSION()) > 0){
				$num = $MySQL->DB_select('COUNT(status)','crm_zadacha z','','iniciator = '.definitionSESSION().' AND status = 2','','','str');
				$answer['count_nw'] = (int)$num;
			} else {
				$answer['count_nw'] = 0;
			}
			
			if(countWorkZ(definitionSESSION()) > 0){
				$num = $MySQL->DB_select('COUNT(status)','crm_zadacha z','','(ispolnitel = '.definitionSESSION().' AND status = 1) OR (ispolnitel = '.definitionSESSION().' AND status = 3)','','','str');
				$answer['count_w'] = (int)$num;
			} else {
				$answer['count_w'] = 0;
			}
	break;
	...
	case 'edit_status' :
			...
			$MySQL->DB_update('crm_zadacha', 'status = '. $status. $filtr, 'id_z = '. $id);
			...
			if(isset($test))
			{
				switch($param){
					...
					case 5	:	if(!empty($RC['id_st']) && !empty($RC['id_is']))
								{
									$id_st = clearData($_POST['id_st'],'i');
									$id_is = clearData($_POST['id_is'],'i');
									switch($id_st)
									{
										case 1: 
										case 3: if($id_is == definitionSESSION())
												{
													$answer['count_w']--;
												}
												break;
	...
	case 'z_notice_w' :	 # задания в работе
			...
			foreach($tasks as $row){
				...
				<tr id='". $row['id_z'] ."'>
					...
					". $dt ."
					<td class='". colorPrioritet($row['prioritet']) ."'>gfhdfh</td>
	...
	case 'prioritet' :
			if(!empty($_POST['id']))
			{
				//проверка ИД на числовое значение
				$id = clearData($_POST['id'],'i');

				//проверка приоритета на числовое значение
				$prioritet = clearData($_POST['prioritet'],'i');
				updatePrioritetZ($prioritet,$id);
			}
	break;		
	...
	case 'newComent' :
			if(!empty($_POST['text']))
			{
				$text_coment = clearData($_POST['text']);
				$date_dob = date('U');
				$id_user = definitionSESSION();
				$id_z = clearData($_POST['id_z'],'i');

				//добавление в базу коментария
				insertZadachaForumComent($date_dob,$text_coment,$id_user,$id_z);
				...
				<div class='".$colorClass."'>
					...
					<div class='clear'></div>
					<div class='text_coment'>".$text_coment."</div>
				</div>
				";
	...
	case 'contentforum' :
			...
			$id = clearData($_POST['id_z'],'i');
			$this_zadacha = selectZadachaForum($id);
			foreach($this_zadacha as $row){
				...
				$select_f = selectZadachaForumFiles($id);
				$conZFile = '';
				...
				<div class='viev_text_zadacha' id='" . $row['id_z'] . "'>
					...
					<div class='clear'></div>	
					<div class='cap_z'><h4>Задача</h4></div>
					<div class='text' >". $row['caption_zadacha'] ."</div>	
					<div class='cap_z'><h4>Описание</h4></div>			
			...
			//выборка данных из из таблицы коментов по ид задачи		
				$All_coment = selectZadachaForumComent($id);
				$contZcom = '';
			...
			{
				$contFomrCon = "
					<p>Комментарий:</p> <textarea rows='7' name='text' required class='form-control'></textarea>
	...
	case 'requere_settings_mgr' :
			...
			$buttonEditProfile = "
			<button name='edit_profile' class='btn btn-default'>
			...
			//{
				$buttonEditGroup = "
				<button name='edit_group' class='btn btn-default'>
					<img src='/task/crm/images/edit_text.png' height='20' width='20'/>
					Создание Менеджера
				</button>
				";
				...
				$buttonViewGroup = "
				<button name='view_group' class='btn btn-default'>
	...
	case 'downEditProfile' :
			...
			foreach($allUsers as $row)
			{
				...
				//$users .= "<option  value='". $row['id'] ."'>". $nameuser ."</option>";
			...
			//$selectUser= "<select multiple class='form-control height500' name='user'>". $users ."</select>";
			...
			<div class='modal modal_message'> 
				<div class='modal-dialog' style='width:400px;'>
					<div class='modal-content'>
						...
						<div class='modal-footer'>
							<button type='button' class='btn btn-default' data-dismiss='modal'>Закрыть</button>
							<button class='btn btn-primary' name='btnCreateMgr' type='button'>Сохранить</button>
						</div>
					</div>
				</div>
			</div>
	...
	case 'uploadGroupMgr':
			...
			if(empty($_POST['userView']) && empty($_POST['userMgr']))
				die('не все данные введены');

			$mgr = clearData($_POST['userMgr'],'i');

			if(!in_array($mgr,$_POST['userView'])){
				$_POST['userView'][] = $mgr;
			}

			$insert = ''; 
			foreach($_POST['userView'] as $row)
			{
				$insert .= "(". $mgr .",". clearData($row,'i') ."),";
			}
			$insert = substr_replace($insert,'',-1); 
			createMgrGroup($insert,$mgr);
	...
	break;
			...
			//////////////////////////////////////////////
			////////  Действия при входе в вкладку задач   ///////
			//////////////////////////////////////////////
	case 'loadTabCRM' :

			/*# проверка на включение подсказок
			if(empty($_COOKIE['Hepls'])) {
				//$helps = selectToHelpUser($_SESSION['userid']);
				//setcookie('Hepls', $helps, time()+99999, '/' );
			}
			# инициализация куки определяющие откуда загружаются файлы
			if(empty($_COOKIE['reqvestUrl'])) {
				setcookie('reqvestUrl', '', time()+99999, '/' );	
			} else {
				$_COOKIE['reqvestUrl'] = '';
			}
			# инициализация куки ИД пользователя 
			if(empty($_COOKIE['id_user'])) {
				setcookie('id_user', $_SESSION['userid'], time()+99999, '/' );
			}
			# инициализация текущей группы пользователя
			if(empty($_COOKIE['id_user'])) {
				$_SESSION['group'] = selectGroupToUser($_SESSION['userid']);
			}*/
	break;
